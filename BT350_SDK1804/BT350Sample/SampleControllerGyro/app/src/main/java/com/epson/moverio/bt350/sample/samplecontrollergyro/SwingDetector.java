...
package com.epson.moverio.bt350.sample.samplecontrollergyro;
...
class SwingDetector implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent event) {
        if (event.sensor.getType() == Sensor.TYPE_CONTROLLER_GYROSCOPE) {
            if (mListener != null) {
    ...
    void startSensor() {
        if (!mIsStartSensor) {
            if (mSensorManager != null) {
                Sensor gyro = mSensorManager.getDefaultSensor(Sensor.TYPE_CONTROLLER_GYROSCOPE);
                if (gyro != null) {
